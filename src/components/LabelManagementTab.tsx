...
const LabelManagementTab = ({ userId, userPlan }: LabelManagementTabProps) => {
  ...
  const fetchLabelData = async () => {
    try {
      ...
      const { data: profile } = await supabase
        .from("profiles")
        .select("id, active_label_id, label_name, email")
        .eq("id", userId)
        .single();
    ...
    <div className="space-y-6">
      ...
      <Card className="backdrop-blur-sm bg-card/80 border-primary/20">
        <CardHeader>
          ...
          <CardDescription>
            {canEditLabel 
              ? "Set your label name for Signature and Prestige members"
              : "Label name is only available for Signature and Prestige members"
            }
          </CardDescription>
        </CardHeader>
